...
//  Hamla_iOS_Captain
//
//  Created by Bassant Elmekkawy on 10/11/2024.
//
...
class SupportChatVC: MessagesViewController {
    ...
    let subTitleLabel: UILabel = {
        let title = UILabel(frame: CGRect(x: 15, y: 25, width: 100, height: 16))
        title.font = UIFont.systemFont(ofSize: 12, weight: .medium)
        title.textColor = UIColor(named: "seagull")
        return title
    }()
    
    let subTitleView: UIView = {
        let view = UIView(frame: CGRect(x: 0, y: 29, width: 8, height: 8))
        view.backgroundColor = UIColor(named: "seagull")
        view.cornerRadius = 4
        return view
    }()
    ...
    private func configureCustomTitle() {
        ...
        titleLabel.text = "Senior Advisor"
        subTitleLabel.text = "Hamla support"
        leftBarButtonView.addSubview(titleLabel)
        leftBarButtonView.addSubview(subTitleView)
    ...
    func bindData(){
        viewModel?.uploadImageResult.bind { [self] result in
            ...
            if result?.status == 0 {
                ...
                FirebaseManager.shared.sendSupportMessage(message: dashboardStorageUrl + data, type: "photo")
            }
...
//        messageInputBar.setStackViewItems([button], forStack: .left, animated: false)
    ...
    func observeNewMessage() {
        FirebaseManager.shared.observeNewSupportMessage { message in
...
extension SupportChatVC: MessagesDataSource {
...
extension SupportChatVC: MessagesLayoutDelegate, MessagesDisplayDelegate {
...
extension SupportChatVC: InputBarAccessoryViewDelegate {
    func inputBar(_ inputBar: InputBarAccessoryView, didPressSendButtonWith text: String) {
        ...
        messagesCollectionView.scrollToLastItem()
        FirebaseManager.shared.sendSupportMessage(message: text, type: "text")
    }
...
extension SupportChatVC: MessageCellDelegate {
...
extension SupportChatVC: PhotoActionSheetDelegate {
